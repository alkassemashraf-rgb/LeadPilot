...
export function KPICard({ label, value, icon: Icon, color, bg, delta, loading }: KPICardProps) {
    if (loading) {
        return (
            <div className="bg-white p-5 rounded-xl border border-slate-100 shadow-sm animate-pulse flex flex-col h-[130px]">
                <div className="h-10 w-10 bg-slate-100 rounded-lg mb-auto"></div>
                <div className="h-8 bg-slate-100 rounded w-1/2 mt-4"></div>
                <div className="h-3 bg-slate-100 rounded w-3/4 mt-2"></div>
            </div>
        );
    }

    return (
        <div className="group bg-white p-5 rounded-xl border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all duration-300 relative overflow-hidden flex flex-col">
            ...
            <div className="relative z-10 mt-auto min-w-0">
                <h3 className="text-3xl font-extrabold tracking-tight text-slate-900 truncate">{value}</h3>
                <p className="text-[11px] mt-1 font-bold text-slate-500 uppercase tracking-widest line-clamp-2 leading-tight">
                    {label}
                </p>
                {delta && (
